...
func draw_battery(posX, posY, bar):
	draw_battery_frame(posX, posY)
	
	var bar_origin_pos = Vector2(posX + BATTERY_HEAD_SIZE.x + (BATTERY_SIZE.x - BATTERY_BAR_SIZE.x), posY + 1)
	for i in range(bar):
		draw_battery_bar(bar_origin_pos.x - i * (BATTERY_BAR_SIZE.x + 1), bar_origin_pos.y)
	
	return

func draw_battery_frame(posX, posY):
...
func draw_battery_bar(posX, posY):
